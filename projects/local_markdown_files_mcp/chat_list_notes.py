...
async def main():
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            user_input = "What notes do I have available?"
            
            # In a full implementation, you'd use a LangChain Agent here
            # For now, let's manually call the list_notes tool
            response = await session.call_tool("list_notes", arguments={})
            print(f"Files found: {response.content}")
